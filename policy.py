...
class Policy(object):
    ...
    def _policy_nn(self):
        ...
        out = tf.layers.dense(out, hid2_size, tf.tanh,
                              kernel_initializer=tf.random_normal_initializer(
                                  stddev=np.sqrt(1 / hid1_size)), name="h3")
        
        out = tf.layers.dense(out, hid3_size, tf.tanh,
                              kernel_initializer=tf.random_normal_initializer(
                                  stddev=np.sqrt(1 / hid2_size)), name="h4")
    ...
    def save(self,num_ep):
        self.saver.save(self.sess,'models/Policy_network',global_step = num_ep)

    def restore(self,num_ep):
        self.saver.restore(self.sess,'models/Policy_network-'+str(num_ep))
